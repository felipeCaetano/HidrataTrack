...
def get_user(identifier, session: Session = get_session()):
    ...
    user_db = (
        session.query(User)
        .filter((User.email == identifier) | (User.login == identifier))
        .first()
    )
...
def save_user(user: User):
    ...
    with get_session() as session:
        try:
            ...
            if existing_user:
                user_events.emit("User-warning", f"Usuário {user.login} já existe.")
                return existing_user
            session.merge(user)
            session.commit()
            user_events.emit("User-events", f"Usuário {user.login} salvo com sucesso.")
            return user
            ...
            user_events.emit("warning", "Ocorreu um erro inesperado. Tente novamente.")
